# Porting from torch(diffusers) to numpy
# Original source: https://github.com/huggingface/diffusers/tree/main/src/diffusers/schedulers/scheduling_euler_ancestral_discrete.py

### patch ###
import numpy as torch
from .scheduling_base import SchedulerMixin, ConfigMixin, register_to_config
from .torch_numpyize import BaseOutput, KarrasDiffusionSchedulers, randn_tensor
...
class EulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        sigmas = np.flip(((1 - self.alphas_cumprod) / self.alphas_cumprod) ** 0.5, 0)
        timesteps = np.linspace(0, num_train_timesteps - 1, num_train_timesteps, dtype=float)[::-1].copy()
        timesteps = torch.from_numpy(timesteps).astype(dtype=torch.float32)
    ...
    def set_timesteps(
        ...
        sigmas = torch.from_numpy(sigmas).astype(dtype=torch.float32)

        # TODO: Support the full EDM scalings for all prediction types and timestep types
        if self.config.timestep_type == "continuous" and self.config.prediction_type == "v_prediction":
            self.timesteps = torch.Tensor([0.25 * sigma.log() for sigma in sigmas[:-1]])
        else:
    ...
    def step(
        ...
        sample = sample.astype(torch.float32)
